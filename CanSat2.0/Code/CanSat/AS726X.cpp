...
void AS726X::begin(TwoWire &wirePort, byte gain, byte measurementMode)
{
	...
	{
		DEBUG_SERIAL.print("ID (should be 0x3E or 0x3F): 0x");
		DEBUG_SERIAL.println(_sensorVersion, HEX);
	}
	...
	if (_sensorVersion == 0)
	{
		DEBUG_SERIAL.println("Sensor failed to respond. Check wiring.");
		while (1); //Freeze!
	}

	if (_sensorVersion == SENSORTYPE_AS7262) DEBUG_SERIAL.println("AS7262 online!");
	if (_sensorVersion == SENSORTYPE_AS7263) DEBUG_SERIAL.println("AS7263 online!");
}
...
void AS726X::printMeasurements()
{
	...
	{
		//Visible readings
		DEBUG_SERIAL.print(" Reading: V[");
		DEBUG_SERIAL.print(getCalibratedViolet(), 2);
		DEBUG_SERIAL.print("] B[");
		DEBUG_SERIAL.print(getCalibratedBlue(), 2);
		DEBUG_SERIAL.print("] G[");
		DEBUG_SERIAL.print(getCalibratedGreen(), 2);
		DEBUG_SERIAL.print("] Y[");
		DEBUG_SERIAL.print(getCalibratedYellow(), 2);
		DEBUG_SERIAL.print("] O[");
		DEBUG_SERIAL.print(getCalibratedOrange(), 2);
		DEBUG_SERIAL.print("] R[");
		DEBUG_SERIAL.print(getCalibratedRed(), 2);
	}
	else if (_sensorVersion == SENSORTYPE_AS7263)
	{
		//Near IR readings
		DEBUG_SERIAL.print(" Reading: R[");
		DEBUG_SERIAL.print(getCalibratedR(), 2);
		DEBUG_SERIAL.print("] S[");
		DEBUG_SERIAL.print(getCalibratedS(), 2);
		DEBUG_SERIAL.print("] T[");
		DEBUG_SERIAL.print(getCalibratedT(), 2);
		DEBUG_SERIAL.print("] U[");
		DEBUG_SERIAL.print(getCalibratedU(), 2);
		DEBUG_SERIAL.print("] V[");
		DEBUG_SERIAL.print(getCalibratedV(), 2);
		DEBUG_SERIAL.print("] W[");
		DEBUG_SERIAL.print(getCalibratedW(), 2);
	}

	DEBUG_SERIAL.print("] tempF[");
	DEBUG_SERIAL.print(tempF, 1);
	DEBUG_SERIAL.print("]");
...
void AS726X::printUncalibratedMeasurements()
{
	...
	if (_sensorVersion == SENSORTYPE_AS7262)
	{
		//Visible readings
		DEBUG_SERIAL.print(" Reading: V[");
		DEBUG_SERIAL.print(getViolet(), 2);
		DEBUG_SERIAL.print("] B[");
		DEBUG_SERIAL.print(getBlue(), 2);
		DEBUG_SERIAL.print("] G[");
		DEBUG_SERIAL.print(getGreen(), 2);
		DEBUG_SERIAL.print("] Y[");
		DEBUG_SERIAL.print(getYellow(), 2);
		DEBUG_SERIAL.print("] O[");
		DEBUG_SERIAL.print(getOrange(), 2);
		DEBUG_SERIAL.print("] R[");
		DEBUG_SERIAL.print(getRed(), 2);
	}
	else if (_sensorVersion == SENSORTYPE_AS7263)
	{
		//Near IR readings
		DEBUG_SERIAL.print(" Reading: R[");
		DEBUG_SERIAL.print(getR(), 2);
		DEBUG_SERIAL.print("] S[");
		DEBUG_SERIAL.print(getS(), 2);
		DEBUG_SERIAL.print("] T[");
		DEBUG_SERIAL.print(getT(), 2);
		DEBUG_SERIAL.print("] U[");
		DEBUG_SERIAL.print(getU(), 2);
		DEBUG_SERIAL.print("] V[");
		DEBUG_SERIAL.print(getV(), 2);
		DEBUG_SERIAL.print("] W[");
		DEBUG_SERIAL.print(getW(), 2);
	}
...
byte AS726X::virtualReadRegister(byte virtualAddr)
{
	...
	{
		//DEBUG_SERIAL.println("Premptive read");
...
byte AS726X::readRegister(byte addr)
{
	...
	if (_i2cPort->available()) {
		...
		DEBUG_SERIAL.println("I2C Error");
		return (0xFF); //Error
	}
}
