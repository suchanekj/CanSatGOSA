/*
	...
	Modified by Josep Virgili to eliminate SFE references to make it more concise and clearer.
...
void BMP180::getData(double &T, double &P)
{
  char status;
  ...
  if (status != 0)
  {
    ...
    if (status != 0)
    {
      ...
      if (status != 0)
      {
        ...
        else DEBUG_SERIAL.println("error retrieving pressure measurement\n");
      }
      else DEBUG_SERIAL.println("error starting pressure measurement\n");
    }
    else DEBUG_SERIAL.println("error retrieving temperature measurement\n");
  }
  else DEBUG_SERIAL.println("error starting temperature measurement\n");
}
