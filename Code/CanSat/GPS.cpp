...
public:
    ...
    void get_status()
    {
        ...
        if (fix().status == gps_fix::STATUS_NONE) {
            ...
            if (!acquiring) {
                acquiring = true;
                dotPrint = millis();
                DEBUG_SERIAL.print( F("Acquiring...") );
            ...
            if (acquiring)
                DEBUG_SERIAL << '\n';
            DEBUG_SERIAL << F("Acquired status: ") << (uint8_t) fix().status << '\n';

#if defined(GPS_FIX_TIME) & defined(GPS_FIX_DATE) & \
            defined(UBLOX_PARSE_TIMEGPS)

            if (!enable_msg( ublox::UBX_NAV, ublox::UBX_NAV_TIMEGPS ))
            DEBUG_SERIAL.println( F("enable TIMEGPS failed!") );
    ...
    void get_leap_seconds()
    {
        ...
        if (GPSTime::leap_seconds != 0) {
          DEBUG_SERIAL << F("Acquired leap seconds: ") << GPSTime::leap_seconds << '\n';

          if (!disable_msg( ublox::UBX_NAV, ublox::UBX_NAV_TIMEGPS ))
            DEBUG_SERIAL.println( F("disable TIMEGPS failed!") );

          #if defined(UBLOX_PARSE_TIMEUTC)
            if (!enable_msg( ublox::UBX_NAV, ublox::UBX_NAV_TIMEUTC ))
              DEBUG_SERIAL.println( F("enable TIMEUTC failed!") );
            state = GETTING_UTC;
          #else
            start_running();
          #endif
    ...
    void get_utc()
    {
        ...
        if (safe && (sow != 0)) {
          DEBUG_SERIAL << F("Acquired UTC: ") << utc << '\n';
          DEBUG_SERIAL << F("Acquired Start-of-Week: ") << sow << '\n';
    ...
    void start_running()
    {
        ...
        if (!enable_msg( ublox::UBX_NAV, ublox::UBX_NAV_POSLLH ))
          DEBUG_SERIAL.println( F("enable POSLLH failed!") );
        ...
        if (!enable_msg( ublox::UBX_NAV, ublox::UBX_NAV_VELNED ))
          DEBUG_SERIAL.println( F("enable VELNED failed!") );
        ...
        if (!enable_msg( ublox::UBX_NAV, ublox::UBX_NAV_DOP ))
          DEBUG_SERIAL.println( F("enable DOP failed!") );
        else
          DEBUG_SERIAL.println( F("enabled DOP.") );
        ...
        if (!enable_msg( ublox::UBX_NAV, ublox::UBX_NAV_SVINFO ))
          DEBUG_SERIAL.println( F("enable SVINFO failed!") );
        ...
        enabled_msg_with_time = true;
          ...
          if (enabled_msg_with_time &&
            ...
            DEBUG_SERIAL.println( F("disable TIMEUTC failed!") );
...
void GPS_init()
{
    ...
    DEBUG_SERIAL.print( F("ublox binary protocol example started.\n") );
    DEBUG_SERIAL << F("fix object size = ") << sizeof(gps.fix()) << '\n';
    DEBUG_SERIAL << F("ubloxGPS object size = ") << sizeof(ubloxGPS) << '\n';
    DEBUG_SERIAL << F("MyGPS object size = ") << sizeof(gps) << '\n';
    DEBUG_SERIAL.println( F("Looking for GPS device on " GPS_PORT_NAME) );
    DEBUG_SERIAL.flush();
...
void GPS_run()
{
    if (gps.available( gpsPort )) {
        fix = gps.read();
        trace_all( DEBUG_SERIAL, gps, gps.read() );
        fix.calculateNorthAndEastVelocityFromSpeedAndHeading();
#ifdef DEBUG
        sprintf(debug_message, "%d-%d-%d %d:%d:%d N%" PRId32 " E%" PRId32 " A%" PRId32 " Velocity N%d E%d D%d.",
                fix.dateTime.year, fix.dateTime.month, fix.dateTime.date,
                fix.dateTime.hours, fix.dateTime.minutes, fix.dateTime.seconds,
                fix.latitudeL(), fix.longitudeL(), fix.altitude_cm(), fix.velocity_north, fix.velocity_east, fix.velocity_down);
        DEBUG_SERIAL.println(debug_message);
#endif
    }
}
